...
class AmoCrmClass {
    ...
    private function getToken($refresh = false): void {
        ...
        $response = json_decode($this->curlRequest($link, $data), true);
    ...
    private function curlRequest($link, $PostFields = []): string {
        $headers = [
    ...
    public function apiPostRequest($service, $params = []): array {
        ...
        try {
            ...
            $result = json_decode($this->curlRequest($url, $params), true);
        } catch (Exception $e) {
            $this->Error($e);
        }
